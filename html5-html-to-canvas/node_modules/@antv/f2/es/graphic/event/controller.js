...
class EventController {
  constructor(_ref) {
    ...
    _defineProperty(this, "_click", ev => {
      var points = convertPoints(ev, this.canvas);
      ev.points = points;
      this.emitEvent('click', ev);
    });

    _defineProperty(this, "_start", ev => {
      var points = convertPoints(ev, this.canvas);
      ...
      ev.points = points;
      this.emitEvent('touchstart', ev); // 防止上次的内容没有清理掉，重新reset下

      this.reset(); // 记录touch start 的时间

      this.startTime = Date.now(); // 记录touch start 的点
    ...
    _defineProperty(this, "_move", ev => {
      ...
      if (points.length > 1) {
        ...
        var direction = this.direction || calcDirection(startPoints[0], points[0]);
        this.direction = direction; // 获取press或者pan的事件类型
        // press 按住滑动, pan表示平移
        // 如果start后立刻move，则触发pan, 如果有停顿，则触发press

        var eventType = this.getEventType(points);
        ev.direction = direction;
        ev.deltaX = deltaX;
        ev.deltaY = deltaY;
        this.emitStart(eventType, ev);
        this.emitEvent(eventType, ev); // 记录最后2次move的时间和坐标，为了给swipe事件用

        var prevMoveTime = this.lastMoveTime;
    ...
    _defineProperty(this, "_end", ev => {
      this.emitEnd(ev);
      this.emitEvent('touchend', ev); // swipe事件处理, 在touchend之后触发
      ...
      if (now - lastMoveTime < 100) {
        ...
        if (intervalTime > 0) {
          var prevMovePoints = this.prevMovePoints || this.startPoints;
          var lastMovePoints = this.lastMovePoints; // move速率
    ...
    _defineProperty(this, "_cancel", ev => {
  ...
  emitEnd(ev) {
    ...
    Object.keys(processEvent).forEach(type => {
      this.emitEvent("".concat(type, "end"), ev);
