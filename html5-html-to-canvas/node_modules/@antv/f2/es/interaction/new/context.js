...
import autoCat from '@antv/scale/lib/auto/cat'; // 判断新老values是否相等，这里只要判断前后是否相等即可
...
class Context {
  ...
  constructor(chart) {
    ...
    _defineProperty(this, "_afterinit", () => {
      // 初始化value值
      var scale = this.getPinchScale(); // 记录原始全量数据

      var values = [].concat(scale.values);
      this.values = values; // 最小的缩放比例
    ...
    _defineProperty(this, "_afterdatachange", () => {
      this.updateRange(this.range);
    });
